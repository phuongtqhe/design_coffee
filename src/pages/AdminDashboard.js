...
export default function Dashboard() {
  ...
  const getStatisticNumber = (from, to) => {
    ...
    const items = orderItems.filter(oi => temp.some(o => o.id === oi.orderId));
    ...
    items.forEach(oi => {
      ...
      const profit = ((oi.unitPrice || prod.price) - (prod.originPrice || 0)) * (oi.quantity || 0);
      data.profit += profit;
      // Category stats
      if (catIdx >= 0) {
        data.category[catIdx].quantity += oi.quantity || 0;
        data.category[catIdx].profit += profit;
        data.category[catIdx].revenue += oi.totalCost || (oi.unitPrice * oi.quantity);
      }
    });
    return data;
  }
  ...
  <input checked type="radio" className="btn-check" name="btnradioxyz" id="btnradiocate1" autoComplete="off" readOnly></input>
