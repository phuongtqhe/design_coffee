...
const productSchema = yup.object({
  ...
  ).required("Must provide at least one image").min(1, 'At least one image is required')
});
...
const AddProduct = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
          setIsLoading(true);
          try {
            const images = await uploadImage(values.product);
            await saveProduct(values, images);
          } catch (error) {
    ...
    const saveProduct = async (values, images) => {
      const { title, price, categoryId, featured, status, description } = values
      
      const now = new Date().toISOString();
      const statusValue = status === true || status === 'active' ? 'active' : 'inactive';
                    ...
                    <Row>
                      ...
                      <Col xs={6} lg={3} className='my-3'>
                        ...
                        <BootstrapForm.Check 
                          ...
                          checked={formik.values.status === 'active' || formik.values.status === true}
                        />
                      </Col>
                      <Col xs={6} lg={3} className='my-3'>
