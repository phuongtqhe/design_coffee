...
function Login() {
  ...
  const handleGoogleLogin = async (credentialResponse) => {
    ...
    const existingSession = sessionStorage.getItem("data");
    if (existingSession) {
      console.log("User already logged in, skipping Google login flow.");
      return;
    }
    ...
    try {
      ...
      const name = decoded.name || "Người dùng Google";
      ...
      if (postRes.ok) {
        ...
        toast.error("Không thể tạo tài khoản Google!");
      }
    } catch (err) {
      console.error(err);
